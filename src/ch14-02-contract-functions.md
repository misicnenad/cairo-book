...
Functions can access the contract's state easily via `self: ContractState`, which abstracts away the complexity of underlying system calls (`storage_read_syscall` and `storage_write_syscall`). The compiler provides two modifiers to decorate `self`: `ref` and `@`. These modifiers are used to distinguish between view and external functions.
