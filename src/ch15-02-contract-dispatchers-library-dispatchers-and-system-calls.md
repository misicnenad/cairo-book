...
Each time a contract interface is defined, two dispatchers are automatically created and exported by the compiler. For the IERC20 interface that we defined in the last chapter, these would be:
...
In this chapter, we are going to discuss what these dispatchers are, how they work, and how to use them.
...
It's also worth noting that all these are abstracted behind the scenes thanks to the power of Cairo plugins.
...
As you can see, we first needed to import `IContractADispatcherTrait` and `IContractALibraryDispatcher` into our contract, which were generated from our interface by the compiler. Then, we created an instance of `IContractALibraryDispatcher` by passing in the `class_hash` of the class we wanted to make library calls to. From there, we were able to call the functions defined in that class, executing its logic in the context of our contract. When we called `set_value` on `ContractA`, it made a library call to the `set_value` function in `IContractA`, updating the `value` storage variable in `ContractA`.
...
To use this syscall, we passed in the contract address, the selector of the function we want to call (see [next section][entry-point-selector]), and the call arguments.
...
[entry-point-selector]: ./ch15-02-contract-dispatchers-library-dispatchers-and-system-calls.md#entry-point-selector
...
Note that in `starknet::call_contract_syscall`, we didn't specify the function name as a string. Instead, we used the `selector!` macro, which computes the `sn_keccak` hash of the provided function signature.
