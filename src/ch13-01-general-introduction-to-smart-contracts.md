...
Starknet contracts are a special superset of Cairo programs, so the concepts previously learned in this book still apply to writing Starknet contracts.
As you may have already noticed, a Cairo program must always have a `main` function that serves as the entry point for the program:
